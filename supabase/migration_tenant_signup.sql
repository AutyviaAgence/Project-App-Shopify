...
BEGIN
  ...
  INSERT INTO profiles (id, email, full_name, avatar_url, tenant_id, subscription_status, trial_ends_at, tokens_limit)
  VALUES (
    NEW.id,
    NEW.email,
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    tokens_limit = COALESCE(profiles.tokens_limit, 200000),
    updated_at = NOW();
  RETURN NEW;
END;
$$;
