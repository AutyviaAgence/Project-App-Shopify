...
export const wabaClient = {
  ...
  /**
   * Envoyer un message PRODUIT UNIQUE (catalogue Meta).
   * Affiche une fiche produit cliquable. Le client peut l'ajouter au panier.
   * Nécessite un catalog_id Meta + le product_retailer_id du produit.
   * À envoyer DANS la fenêtre 24h (message interactif, pas un template).
   */
  sendSingleProduct(
    phoneNumberId: string,
    accessToken: string,
    to: string,
    params: { catalogId: string; productRetailerId: string; bodyText?: string; footerText?: string }
  ) {
  ...
  /**
   * Envoyer un MULTI-PRODUCT MESSAGE (liste de produits du catalogue Meta).
   * Le client parcourt les produits, les ajoute au panier et envoie une
   * commande (reçue via webhook en message type "order").
   * `sections` : 1 à 10 sections, chacune ≤ 30 produits (product_retailer_ids).
   */
  sendProductList(
    phoneNumberId: string,
    accessToken: string,
