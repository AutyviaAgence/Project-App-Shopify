...
// Opt-in WhatsApp PENDANT le checkout (sous le champ contact). Capture le
// consentement AVANT un éventuel abandon → permet la relance de panier abandonné.
export default extension('purchase.checkout.contact.render-after', (root, api) => {
  ...
  // Pré-remplissage DANS LE CHECKOUT : le client saisit son téléphone dans le
  // formulaire → on y a accès directement (buyerIdentity / shippingAddress),
  // sans appel serveur. On suit la valeur en continu (elle peut être saisie après
  // l'affichage de l'extension).
  function applyPhone(raw) {
    const digits = (raw || '').toString().replace(/\D/g, '')
    if (!digits || localNumber) return
  ...
  // Sources possibles du numéro au checkout.
  applyPhone(api.buyerIdentity?.phone?.current || api.shippingAddress?.current?.phone)
  // S'abonner aux changements (le client peut remplir le tel après coup).
  try {
    api.buyerIdentity?.phone?.subscribe?.((v) => applyPhone(v))
    api.shippingAddress?.subscribe?.((addr) => applyPhone(addr?.phone))
  } catch { /* certaines surfaces n'exposent pas subscribe : saisie manuelle */ }
